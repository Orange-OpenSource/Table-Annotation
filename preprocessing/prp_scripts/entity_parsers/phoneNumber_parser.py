...
## verify whether each cell in list_cell reprensents a PHONE NUMBER
def phonenumber_parser(list_cell):
    ner_per_label = {}
    for label in list_cell:
        ner_per_label[label] = []
        # for code in country_codes:
        try:
            phone_res = phonenumbers.parse(label)
            if phonenumbers.is_valid_number(phone_res):
                ner_per_label[label].append("PHONE NUMBER")
                break
        except:
            pass
